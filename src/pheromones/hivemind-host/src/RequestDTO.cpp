...
RequestDTO::RequestDTO(const Request& request) : m_id(request.id) {
    ...
    case Request_userCall_tag:
        m_request = UserCallRequestDTO(request.message.userCall);
        break;
    case Request_hiveApi_tag:
        m_request = HiveMindApiRequest(request.message.hiveApi);
        break;
    case Request_swarm_tag:
        m_request = SwarmApiRequest(request.message.swarm);
        break;
    default:
...
bool RequestDTO::serialize(Request& request) const {
    ...
    if (const auto* userRequest = std::get_if<UserCallRequestDTO>(&m_request)) {
        request.which_message = Request_userCall_tag;
        return userRequest->serialize(request.message.userCall);
    }

    if (const auto* hiveRequest = std::get_if<HiveMindApiRequestDTO>(&m_request)) {
        request.which_message = Request_hiveApi_tag;
        return hiveRequest->serialize(request.message.hiveApi);
    }

    if (const auto* swarmRequest = std::get_if<SwarmApiRequestDTO>(&m_request)) {
        request.which_message = Request_swarm_tag;
        return swarmRequest->serialize(request.message.swarm);
    }
