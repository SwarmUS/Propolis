...
HiveConnectHiveMindApiDTO::HiveConnectHiveMindApiDTO(const HiveConnectHiveMindApi& apiMsg) :
    ...
    case HiveConnectHiveMindApi_network_config_set_request_tag:
        m_message =
            HiveConnectNetworkConfigSetRequestDTO(apiMsg.message.network_config_set_request);
        break;
    case HiveConnectHiveMindApi_network_config_get_request_tag:
        m_message =
            HiveConnectNetworkConfigGetRequestDTO(apiMsg.message.network_config_get_request);
        break;
    case HiveConnectHiveMindApi_network_config_set_response_tag:
        m_message =
            HiveConnectNetworkConfigSetResponseDTO(apiMsg.message.network_config_set_response);
        break;
    case HiveConnectHiveMindApi_network_config_get_response_tag:
        m_message =
            HiveConnectNetworkConfigGetResponseDTO(apiMsg.message.network_config_get_response);
        break;
    default:
...
HiveConnectHiveMindApiDTO::HiveConnectHiveMindApiDTO(
    uint32_t id, const HiveConnectNetworkConfigGetRequestDTO& req) :
    m_messageId(id), m_message(req) {}

HiveConnectHiveMindApiDTO::HiveConnectHiveMindApiDTO(
    uint32_t id, const HiveConnectNetworkConfigSetRequestDTO& req) :
    m_messageId(id), m_message(req) {}

HiveConnectHiveMindApiDTO::HiveConnectHiveMindApiDTO(
    uint32_t id, const HiveConnectNetworkConfigGetResponseDTO& res) :
    m_messageId(id), m_message(res) {}

HiveConnectHiveMindApiDTO::HiveConnectHiveMindApiDTO(
    uint32_t id, const HiveConnectNetworkConfigSetResponseDTO& res) :
    m_messageId(id), m_message(res) {}
...
bool HiveConnectHiveMindApiDTO::serialize(HiveConnectHiveMindApi& apiMsg) const {
    ...
    if (const auto* configSetRequest =
            std::get_if<HiveConnectNetworkConfigSetRequestDTO>(&m_message)) {
    ...
    if (const auto* configGetRequest =
            std::get_if<HiveConnectNetworkConfigGetRequestDTO>(&m_message)) {
    ...
    if (const auto* configSetResponse =
            std::get_if<HiveConnectNetworkConfigSetResponseDTO>(&m_message)) {
    ...
    if (const auto* configGetResponse =
            std::get_if<HiveConnectNetworkConfigGetResponseDTO>(&m_message)) {
